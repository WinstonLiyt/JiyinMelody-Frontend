        ...
        <div style="flex: 1; width: 0">
          ...
          <div class="card" style="padding: 30px; margin-bottom: 10px">
            ...
            <div style="color: #666; margin-bottom: 20px">
              <span style="margin-right: 20px"><i class="el-icon-user"></i> {{ this.blog.author.nickname|| "用户"+this.blog.author.id
                }}</span>
            ...
            <div class="card" style="text-align: center; font-size: 20px; color: #666; margin-bottom: 10px">
              <span style="margin-right: 20px; cursor: pointer;" @click="setlike"
                :class="{ 'active': blog.is_liked }"><i class="el-icon-like"></i> {{ blog.like_count }}</span>
              <span style="margin-right: 20px; cursor: pointer" @click="setcollect" :class="{ 'active': blog.is_collected }"><i
                  class="el-icon-star-off"></i> {{ blog.collect_count }}</span>
              <span style=" cursor: pointer" @click="openshareblog"><i
                  class="el-icon-share"></i> </span>
                ...
                <div style="display: flex; grid-gap: 20px; margin-bottom: 20px" v-for="(item,index) in flagcomments"
                  ...
                  <div style="flex: 1">
                    ...
                    <div style="margin-bottom: 10px">
                      <div style="color: #666; margin-bottom: 10px">{{ item.author.nickname||"用户"+item.author.id }}</div>
                      ...
                      <div style="color: #888; font-size: 13px; margin-bottom: 10px"><span style="margin-right: 20px">{{
                          item.timestamp }}</span>
                        <span style="cursor: pointer;" :class="{ 'comment-active': item.isreply }"
                          @click="reply(item)"><i class="el-icon-s-comment"></i>评论</span>
                    ...
                    <div style="display: flex;  grid-gap: 20px; margin-bottom: 20px"
                      ...
                      <div style="flex: 1">
                        <div style="color: #666; margin-bottom: 10px">{{ citem.author.nickname || "用户"+citem.author.id }} <span
                            style="color: #333" v-if="findcommentauthor(citem.parent_comment_id).id !== user.id">回复 @{{
                            findcommentauthor(citem.parent_comment_id).author.nickname||"用户"+ findcommentauthor(citem.parent_comment_id).author.id }}</span></div>
                        <div style="color: #444; margin-bottom: 10px">{{ citem.content }}</div>
                        <div style="color: #888; font-size: 13px; margin-bottom: 10px"><span
                            style="margin-right: 20px">{{ citem.timestamp }}</span>
  ...
  this.$router.push({ name: 'OtherView', params: { id: sendid } });
    ...
    setlike(){
      ...
      set_like({
                 ...
                 is_liked: this.blog.is_liked}).then(response => {
          console.log(response.data);
          if (response.data.code === 200) {
    ...
    setcollect(){
      ...
      set_collect({
                 blog_id: this.blog.id,
                 is_collected: this.blog.is_collected}).then(response => {
          console.log(response.data);
          if (response.data.code /*=== 200*/) {
    ...
    submitcomments(index){
      ...
      if(index)
      { 
        submitcomments({ blog_id: this.blog.id,
